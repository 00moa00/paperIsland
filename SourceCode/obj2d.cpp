...
void Collider::hit_b(OBJ2D* obj)
{
   // BG* bg = obj->bg();
    Transform* t = obj->transform();
    //Collider* coll = obj->collider();
    ActorComponent* act = obj->actorComponent();

    if (t->getPositionY() <= attackBox_.top + size_.y / 2)
    {

        act->set_BlockFlagDo(true);
    }

    else if (t->getPositionY() >= attackBox_.bottom - size_.y / 2)
    {

        act->set_BlockFlagUp(true);
    }

    else if (t->getPositionX() < attackBox_.left + size_.x / 2)
    {
        act->set_BlockFlagRight(true);
    }
    else if (t->getPositionX() > attackBox_.right - size_.x / 2)
    {
        act->set_BlockFlagLeft(true);
    }

}

void Collider::hit_r(OBJ2D* obj)
{
    BG* bg = obj->bg();
    Transform* t = obj->transform();
    Collider* coll = obj->collider();
    ...
    else if (t->getPositionY() >= attackBox_.bottom)
    {

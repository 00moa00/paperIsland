...
void ShurikenBehavior::update(OBJ2D* obj)
{
    ...
    float angle = obj->weaponComponent()->xFlip() ? -getParam()->ROT_SPEED : getParam()->ROT_SPEED;
...
{
    OBJ2D* owner = obj->weaponComponent()->owner();

    // 角度
    bool xFlip = owner->actorComponent()->xFlip();
    float angle = 0.0f;
    if (xFlip)
    {
        angle = 30.0f + obj->timer() * -15.0f;
    ...
    obj->transform()->setRotation(ToRadian(angle));

    // 位置
    VECTOR2 pos = owner->transform()->position() + VECTOR2(0, -48);
